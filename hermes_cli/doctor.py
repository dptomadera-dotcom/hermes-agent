...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    The connectivity probes use human-facing labels (for example
    ``Google AI Studio`` and ``MiniMax``), while auth status may be keyed by a
    provider's canonical/internal name (for example ``gemini`` or ``xai``).
    Keep a small alias set here so healthy OAuth auth suppresses only the final
    blocking issue list, regardless of which display label the probe emitted.
    """
