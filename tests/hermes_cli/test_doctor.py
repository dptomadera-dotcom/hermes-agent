...
def test_run_doctor_gemini_uses_x_goog_api_key_header(monkeypatch, tmp_path):
    ...
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
    (home / ".env").write_text("GEMINI_API_KEY=gm-test\n", encoding="utf-8")
    project = tmp_path / "project"
    project.mkdir(exist_ok=True)
    ...
    def fake_get(url, headers=None, timeout=None):
        ...
        if url == "https://generativelanguage.googleapis.com/v1beta/models":
            assert headers.get("x-goog-api-key") == "gm-test"
...
class TestGitHubTokenCheck:
    ...
    def test_no_token_and_not_gh_authenticated_stays_quiet(self, monkeypatch, tmp_path):
        home = tmp_path / ".hermes"
        home.mkdir(parents=True, exist_ok=True)
